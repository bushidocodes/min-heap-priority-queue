...
void initialize_should_set_first_free_to_1()
{
  TEST_ASSERT_EQUAL_INT32(1, pq.first_free);
}

void initialize_should_set_highest_priority_to_ULONG_MAX()
{
  TEST_ASSERT_EQUAL_UINT64(ULONG_MAX, pq.highest_priority);
}

void length_should_be_one_less_than_first_free()
{
  TEST_ASSERT_EQUAL_INT32(pq.first_free - 1, priority_queue_length(&pq));
}

void enqueue_should_increment_first_free_and_length()
{
...
void enqueue_first_call_should_set_highest_priority()
{
  struct sandbox_request *sandbox_one = sandbox_request_allocate(10);
  priority_queue_enqueue(&pq, sandbox_one);
...
void enqueue_first_call_should_set_index_1()
{
  struct sandbox_request *sandbox_one = sandbox_request_allocate(10);
  priority_queue_enqueue(&pq, sandbox_one);
...
void enqueue_returns_neg1_on_full()
{
  struct sandbox_request *sandbox_one = sandbox_request_allocate(10);
  ...
  for (int i = 0; i < MAX - 1; i++)
    TEST_ASSERT_EQUAL_INT32(0, priority_queue_enqueue(&pq, sandbox_one));
...
void dequeue_on_empty_returns_null()
{
  TEST_ASSERT_EQUAL_PTR(NULL, priority_queue_dequeue(&pq));
}

void dequeue_last_element_should_set_ULONG_MAX()
{
  TEST_ASSERT_EQUAL_UINT64(ULONG_MAX, pq.highest_priority);
  struct sandbox_request *sandbox_one = sandbox_request_allocate(10);
  priority_queue_enqueue(&pq, sandbox_one);
...
void dequeue_should_return_in_priority_order()
{
